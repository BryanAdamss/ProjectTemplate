...
(function(factory) {
    ...
    $.fn.c_carousel = function(options) {
        $config = $.extend({}, settings, options); // 保存配置
        return this.each(function() {
            var $this = $(this), // 每个Banner
                bannerList = $this.find('.c-ImgSlider-list');
            $this.height($config.height); // 设置容器高度
            slider.listItems = bannerList.find('.c-ImgSlider-item');
            ...
            bannerList.on('click', '.c-ImgSlider-item', function() {
                location.href = $(this).data('href');
            });

            if ($config.showPagination === true) { // 生成、显示pagination并绑定事件
                var page = $('<div class="c-ImgSlider-pagination"></div>');
                for (var i = 0, span = ''; i < slider.count; i++) {
                    if (i === $config.startIndex) {
                        span += '<span class="c-ImgSlider-paginationItem is-cur"></span>';
                        slider.curIndex = i;
                        continue;
                    }
                    span += '<span class="c-ImgSlider-paginationItem"></span>';
                }
                page.html($(span));
                ...
                slider.pageItems = $('.c-ImgSlider-paginationItem')
                page.on('click', '.c-ImgSlider-paginationItem', function() {
                    clearInterval(timer);
