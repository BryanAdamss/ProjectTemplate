    ...
    $.fn.c_carousel = function(options) {
        ...
        return this.each(function() {
            var $this = $(this), // 每个Banner
                bannerList = $this.find('.c-Carousel-list');
            $this.height($config.height); // 设置容器高度
            slider.listItems = bannerList.find('.c-Carousel-item');
            ...
            bannerList.on('click', '.c-Carousel-item', function() {
                location.href = $(this).data('href');
            });

            if ($config.showPagination === true) { // 生成、显示pagination并绑定事件
                var page = $('<div class="c-Carousel-pagination"></div>');
                for (var i = 0, span = ''; i < slider.count; i++) {
                    if (i === $config.startIndex) {
                        span += '<span class="c-Carousel-paginationItem is-cur"></span>';
                        slider.curIndex = i;
                        continue;
                    }
                    span += '<span class="c-Carousel-paginationItem"></span>';
                }
                page.html($(span));
                ...
                slider.pageItems = $('.c-Carousel-paginationItem')
                page.on('click', '.c-Carousel-paginationItem', function() {
                    clearInterval(timer);
